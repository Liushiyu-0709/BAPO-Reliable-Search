...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def allgather_list(self, data: list):
        """
        AllGather a list across processes.
        ...
        Returns:
            Flattened list containing data from all processes
        """
        size = vllm_ps.get_tensor_model_parallel_world_size()
        group=vllm_ps.get_tensor_model_parallel_group()
        # 创建用于接收的列表
        gathered_list = [None for _ in range(size)]

        # 使用 all_gather_object 收集 Python 对象
        torch.distributed.all_gather_object(gathered_list, data, group=group)
        
        # 展平列表：[[rank0_data], [rank1_data], ...] -> [item1, item2, ...]
        output = [item for sublist in gathered_list for item in sublist]
