...
def check_answer_consistence(norm_answer_ls, is_rejection_ls):
    ...
    return len(set(norm_answer_no_rejection)) <= 3 #如果都是答案集合较小，说明解法比较重复但都是错的，选择鼓励IDK,如果答案集合比较大，说明仍然在探索

def compute_score(tokenizer, solution_str_ls, ground_truth_ls, step, stage_level=True, sample_level=False):
    ...
    if step <= 20: #前20步先不鼓励idk
        return scores, reasons, [0 for i in range(len(scores))]
    for i in range(len(is_rejection_ls)):
        if is_rejection_ls[i]:
            scores[i] = 0.5
            reasons[i] = "i don't know should be encouraged"
    rollout_size = 8

    advantages = []
    for group_start in range(0, len(scores), rollout_size): #这里是rollout的数量，默认为8了
