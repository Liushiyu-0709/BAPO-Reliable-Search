...
# Extract whether this is a rejection case
def is_rejection(solution_str):
    """
    Determine whether the model outputs rejection content
    """
    # Extract the assistant's response part
    if "<|im_start|>assistant\n" in solution_str:
        response = solution_str.split("<|im_start|>assistant\n")[1]
    else:
        response = solution_str.split("Assistant:")[-1] if "Assistant:" in solution_str else solution_str
    
    # Convert to lowercase for easier matching
    extract_response = extract_answer(response)
    if extract_response == None:
        return False
    
    response_lower = extract_response.lower()
    ...
    # Check if rejection patterns are present
...
def check_answer_consistence(norm_answer_ls, is_rejection_ls):
    
    norm_answer_no_rejection = [norm_answer_ls[i] for i in range(len(is_rejection_ls)) if not is_rejection_ls[i]]
    return len(set(norm_answer_no_rejection)) <= 3 # If answer set is small, solutions are repetitive but wrong, encourage IDK; if answer set is large, still exploring

def compute_score(tokenizer, solution_str_ls, ground_truth_ls, step, stage_level=True, sample_level=True, idk_ratio=0.05, early_stage=True):
    scores, reasons, is_rejection_ls, norm_answer_ls = [], [], [], []
    for i in range(len(solution_str_ls)):
        solution_str = solution_str_ls[i]
        ground_truth = ground_truth_ls[i]
        score, reason = compute_score_single(tokenizer, solution_str, ground_truth)
        scores.append(score)
        
        if is_rejection(solution_str):
            is_rejection_ls.append(True)
        else:
        ...
        if answer_part is not None:
            ...
            answer = ""
        norma_answer = normalize_answer(answer)
        norm_answer_ls.append(norma_answer)
    
    if step == 0:
        return scores, reasons, [0 for i in range(len(scores))]

    alpha = idk_ratio
    # If using stage_level modulator, in early stage and idk_rate is greater than threshold alpha
    if stage_level:
        if early_stage and sum(is_rejection_ls) / len(is_rejection_ls) >= alpha:
            print("step:", step, "rejection rate:", sum(is_rejection_ls) / len(is_rejection_ls), "not add idk reward")
            return scores, reasons, [0 for i in range(len(scores))]
    advantages = []
    rollout_size = 8
    
    for group_start in range(0, len(scores), rollout_size): # This is the rollout size, default is 8
        # 1. Ensure all 8 ground_truth_ls from i to i+7 are the same (indicating the same question)
        ...
        if len(set(normalized_ground_truths)) != 1:
            raise ValueError(f"Warning: Group {group_start//rollout_size} has different ground truths: {set(group_ground_truths)}")
        
        # 2. Check if all scores in this group are <= 0
        group_scores = scores[group_start:group_end]
        if all(score <= 0 for score in group_scores):
            # if 
            if (stage_level and not early_stage) and sample_level and not check_answer_consistence(norm_answer_ls[group_start:group_end], is_rejection_ls[group_start:group_end]):
                print("all answers are not consistent:", norm_answer_ls[group_start:group_end])
            else:
        ...
        group_mean = torch.mean(torch.tensor(scores[group_start:group_end], dtype=float))
        group_std = torch.std(torch.tensor(scores[group_start:group_end], dtype=float))

        group_scores_normalized = (torch.tensor(scores[group_start:group_end]) - group_mean) / group_std + 1e-6
        advantages.extend(group_scores_normalized.tolist())

    refuse_advs, answer_advs = 0.0, 0.0
    for i in range(len(scores)):
        if "i don't know" in reasons[i] and not np.isnan(advantages[i]):
            refuse_advs += advantages[i]
        elif "i don't know" not in reasons[i] and not np.isnan(advantages[i]):
            answer_advs += advantages[i]
    print("refuse advs:", refuse_advs)
    print("answer advs:", answer_advs)

    return scores, reasons, advantages

    
    
def compute_score_single(tokenizer, solution_str, ground_truth) -> float:
